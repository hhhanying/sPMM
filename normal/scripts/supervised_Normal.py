...
def CV_supervised_Normal(X, Y, Nfold, T, mu_Mu, sigma2_Mu, alpha_Lambda, beta_Lambda, K, b, alpha, ntrace, nchain, nskip):
    ...
    for train_index, test_index in kf.split(X):
        tem_accuracy = accuracy_supervised_Normal(X[train_index], Y[train_index], X[test_index], Y[test_index], T, mu_Mu, sigma2_Mu, alpha_Lambda, beta_Lambda, K, b, alpha, ntrace, nchain, nskip)
